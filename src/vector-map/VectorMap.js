...
/**
 * Represents a two-dimensional grid of nodes in a pathfinding graph.
 *
 * VectorMap manages a 2D plane of nodes, providing utilities for node lookup, traversability
 * validation, and path visualization. It supports creation from emoji strings or 2D arrays,
 * and maintains configurable traversability rules via keys.json to determine which nodes and
 * connections are valid for pathfinding algorithms.
 *
 * @class
 */
class VectorMap {
        ...
        switch (Math.sign(x1 - x2)) {
          case -1:
            switch (Math.sign(y1 - y2)) {
              ...
              /* istanbul ignore next */
              default: throw new Error('VectorMap: unknown path for traversal');
            } break;
