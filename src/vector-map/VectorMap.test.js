...
describe('VectorMap', () => {
    ...
    describe('twoDimensionalArrayToVectorMap', () => {
      test('throws an error when the arrays are not all the same length', () => {
        ...
        expect(() => VectorMap.twoDimensionalArrayToVectorMap(invalidArray))
          .toThrow('VectorMap: cannot convert arrays with non-standard length');
      });

      test('throws an error when a n index includes an unknown key', () => {
        const invalidArray = [
          ['🟩', '🟩', '🟩', '⬛️', '⬛️', '⬛️', '⬛️'],
          ['🟩', '🟩', '🟩', '🙅‍♂️', '🟩', '🟩', '🟩'],
          ['⬛️', '🟩', '🟩', '⬛️', '🟩', '🟩', '⬛️']
        ...
        expect(() => VectorMap.twoDimensionalArrayToVectorMap(invalidArray))
          .toThrow('VectorMap: attempted to parse unknown key: [🙅‍♂️] at (1, 3)');
      });
  ...
  describe('printTraversal', () => {
    ...
    test('throws an error for invalid paths', () => {
      ...
      ])).toThrow('VectorMap: unknown path for traversal');
    });
    ...
    test('returns a modified VectorMap print with the path traversal', () => {
      expect(vectorMapPrint.printTraversal([
        ...
        new Vector(vectorMapPrint.findNode(5, 5), vectorMapPrint.findNode(5, 5), 1),
  ...
  describe('logTraversal', () => {
    ...
    test('logs a printable sting representing the vector map', () => {
      ...
      expect(console.log).toHaveBeenCalledWith(vectorMapString);
    });
    ...
    test('logs a printable sting representing the path traversal on the vector map', () => {
      expect(vectorMapLog.logTraversal(path)).toBeUndefined();
      // eslint-disable-next-line no-console
      expect(console.log).toHaveBeenCalledWith(traversalString);
    });
  });
